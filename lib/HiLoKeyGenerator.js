...
HiLoKeyGenerator.prototype.generateDocumentKey = function (entityName, callback) {
  ...
  self._nextId(entityName, function (error, id) {
    if (error) { return callback(error, null); }
    
    callback(undefined, { key: entityName + self.keySeparator + id });
  });
};
 ...
 */
HiLoKeyGenerator.prototype._nextId = function (entityName, callback) {
  var self = this
    , generator;
  ...
  if (generator.lastId > generator.currentMax) {
    ...
    self._getNextMax(entityName, function (error) {
  ...
  self.client.getDocument(self.documentPrefix + entityName, function (error, result) {
    ...
    if (doc) {
      ...
      self.client.putDocument(self.documentPrefix + entityName, doc, function (error, result) {
        if (error || !result) { return callback(error); }
        if (!result.ok) { return callback(new Error('Unable to update HiLO metadata')); }
      ...
      self.client.putDocument(self.documentPrefix + entityName, doc, function (error, result) {
        if (error || !result) { return callback(error); }
        if (!result.ok) { return callback(new Error('Unable to update HiLoKeyGenerator metadata')); }
