...
Client.prototype.ensureDatabaseExists = function (name, callback) {
  ...
  self.getDocument(docId, { isRoot : true }, function (error, result) {
    if (error || !result) { return callback(error, result.response, false); } // Error connecting to server
    if (result.document) { return callback(error, result.response, true); } // Document and therefore database already exists
