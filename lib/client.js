 ...
 * databaseName - Optionall pass the database name to have the server url created for you
...
function Client(options) {
  ...
  // Trim off trailing / in server_url
  if (options.server_url.charAt(options.server_url.length - 1) === '/') {
    options.server_url = options.server_url.substring(0, options.server_url.length - 1);
  }
  ...
  this.httpClient = ravenHttpClient({ server_url: this.server_url, auth_user: this.auth_user, auth_password: this.auth_password });
...
Client.prototype.queryIndex = function (indexName, options, callback) {
  var self = this
    , queryOptions = { 'query' : self.buildRavenQuery(options.query) }
    ;
...
Client.prototype.useDatabase = function (name) {
  this.server_url += '/databases/' + name;
};
